...
import { AnimationRepo } from "../animation.repo";

@provide(IOC_TYPE.AnimationRepoImpl)
export class AnimationRepoImpl implements AnimationRepo {
  async selectAllBy(filters) : Promise<any> {
    const con = await createConnection({...ormSGWConnParam, entities: [Animation]});
    ...
    try {
      ...
      const repo = con.repositoryFor<Animation>("Animation");
