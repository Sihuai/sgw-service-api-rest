...
import { ShopCategoryRepo } from "../shop.category.repo";

@provide(IOC_TYPE.ShopCategoryRepoImpl)
export class ShopCategoryRepoImpl implements ShopCategoryRepo {
  async selectAll() : Promise<any> {
    const con = await createConnection({...ormSGWConnParam, entities: [ShopCategory]});

    try {
      const repo = con.repositoryFor<ShopCategory>("ShopCategory");
