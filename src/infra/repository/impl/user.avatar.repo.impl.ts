...
import { UserAvatarRepo } from "../user.avatar.repo";

@provide(IOC_TYPE.UserAvatarRepoImpl)
export class UserAvatarRepoImpl implements UserAvatarRepo {
  async selectAll() : Promise<any> {
    const con = await createConnection({...ormSGWConnParam, entities: [UserAvatar]});

    try {
      const repo = con.repositoryFor<UserAvatar>("UserAvatar");
