...
@provide(IOC_TYPE.EditAnimationAction, true)
@provide('action', true)
export class EditAnimationAction implements IAction {
  payloadExample = ` `;
  description = '';
  constructor(
  ...
  async execute(token, request: IRequest) : Promise<any> {
    ...
    if (isEmptyObject(request._key) == true) return -128; // Animation key is empty!
    ...
    return await this.animationService.editOne(model);
  }
}