...
@provide(IOC_TYPE.RemoveFromProductAction, true)
@provide('action', true)
export class RemoveFromProductAction implements IAction {
  ...
  execute(token, request: IGenericEdgeDTO) {
    ...
    if (isEmptyObject(request.tokey) == true) return -2; // Product Key is empty!
    
    const model = new GenericEdge();
    model._from = 'Coupon/' + request.fromkey;
    model._to = 'Product/' + request.tokey;
